...
namespace Cowboy.Sockets
{
    public sealed class AsyncTcpSocketSession
    {
        ...
        private async Task Close(bool shallNotifyUserSide)
        {
            ...
            {
                _log.Debug("Session closed for [{0}] on [{1}] in dispatcher [{2}] with session count [{3}].",
                    this.RemoteEndPoint,
        ...
        {
            _log.Error(string.Format("Session [{0}] error occurred in user side [{1}].", this, ex.Message));
