    ...
    public class SequentialRingBuffer : RingBuffer
    {
        ...
        public override void Put(byte[] buffer, int offset, int count)
        {
            ...
            while (count > 0) {
                ...
                Buffer.BlockCopy(buffer, offset, BufferMemory, BufferTailOffset, chunk);
        ...
        public override int PutFrom(Stream source, int count)
        {
            ...
            while (remaining > 0) {
                ...
                int chunkIn = 0;
                while (chunkIn < chunk) {
                    int iterIn = source.Read(BufferMemory, BufferTailOffset, chunk - chunkIn);
                    if (iterIn < 1) {
        ...
        public override async Task<int> PutFromAsync(Stream source, int count, CancellationToken cancellationToken)
        {
            ...
            while (remaining > 0) {
                ...
                int chunkIn = 0;
                while (chunkIn < chunk) {
                    int iterIn = await source.ReadAsync(BufferMemory, BufferTailOffset, chunk - chunkIn, cancellationToken);
        ...
        public override byte Take()
        {
            ...
            byte output = BufferMemory[BufferHeadOffset++];
        ...
        public override void Take(byte[] buffer, int offset, int count)
        {
            ...
            while (count > 0) {
                ...
                Buffer.BlockCopy(BufferMemory, BufferHeadOffset, buffer, offset, chunk);
        ...
        public override void TakeTo(Stream destination, int count)
        {
            ...
            while (count > 0) {
                ...
                destination.Write(BufferMemory, BufferHeadOffset, chunk);
                BufferHeadOffset = (BufferHeadOffset + chunk == Capacity) ? 0 : BufferHeadOffset + chunk;
                ContentLength -= chunk;
                count -= chunk;
            }
        ...
        public override async Task TakeToAsync(Stream destination, int count, CancellationToken cancellationToken)
        {
            ...
            while (count > 0) {
                int chunk = Math.Min(Capacity - BufferHeadOffset, count);
                await destination.WriteAsync(BufferMemory, BufferHeadOffset, chunk, cancellationToken);
                if (cancellationToken.IsCancellationRequested) {
                    return;
                }
                BufferHeadOffset = (BufferHeadOffset + chunk == Capacity) ? 0 : BufferHeadOffset + chunk;
