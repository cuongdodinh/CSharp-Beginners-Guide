...
namespace C3SockNetUtil.RingByteBuffer
{
    /// <summary>
    ...
    public class ConcurrentRingBuffer : RingBuffer
    {
        ...
        public override void Put(byte input)
        {
            ...
            try {
                ...
                // Write shared state
                BufferMemory[BufferTailOffset++] = input;
        ...
        public override void Put(byte[] buffer, int offset, int count)
        {
            ...
            while (length > 0) {
                ...
                Buffer.BlockCopy(buffer, offset, BufferMemory, localBufferTailOffset, chunk);
        ...
        public async Task PutAsync(byte[] buffer, int offset, int count)
        {
            ...
            while (length > 0) {
                ...
                ts[tsi] = Task.Run(() => Buffer.BlockCopy(buffer, offsetClosure, BufferMemory, tailOffsetClosure, chunk));
                localBufferTailOffset = (localBufferTailOffset + chunk == Capacity) ? 0 : localBufferTailOffset + chunk;
                offset += chunk;
                length -= chunk;
                tsi++;
        ...
        public override int PutFrom(Stream source, int count)
        {
            ...
            while (remaining > 0 || !earlyFinish) {
                ...
                int chunkIn = 0;
                while (chunkIn < chunk) {
                    int iterIn = source.Read(BufferMemory, localBufferTailOffset, chunk - chunkIn);
                    if (iterIn < 1) {
                        earlyFinish = true;
                    }
                    chunkIn += iterIn;
        ...
        public override void PutExactlyFrom(Stream source, int count)
        {
            ...
            while (length > 0) {
                ...
                int chunkIn = 0;
                while (chunkIn < chunk) {
                    var iterIn = source.Read(BufferMemory, localBufferTailOffset, chunk - chunkIn);
                    if (iterIn < 1) {
        ...
        public override async Task<int> PutFromAsync(Stream source, int count, CancellationToken cancellationToken)
        {
            ...
            while (remaining > 0 || !earlyFinish) {
                ...
                int chunkIn = 0;
                while (chunkIn < chunk) {
                    int iterIn = await source.ReadAsync(BufferMemory, localBufferTailOffset, chunk - chunkIn, cancellationToken);
        ...
        public override byte Take()
        {
            ...
            try {
                ...
                // Read and update shared state
                output = BufferMemory[BufferHeadOffset++];
        ...
        public override void Take(byte[] buffer, int offset, int count)
        {
            ...
            while (length > 0) {
                ...
                Buffer.BlockCopy(BufferMemory, localBufferHeadOffset, buffer, offset, chunk);
        ...
        public async Task TakeAsync(byte[] buffer, int offset, int count)
        {
            ...
            while (length > 0) {
                ...
                ts[tsi] = Task.Run(() => Buffer.BlockCopy(BufferMemory, headOffsetClosure, buffer, offsetClosure, chunk));
                localBufferHeadOffset = (localBufferHeadOffset + chunk == Capacity) ? 0 : localBufferHeadOffset + chunk;
                offset += chunk;
                length -= chunk;
                tsi++;
        ...
        public override void TakeTo(Stream destination, int count)
        {
            ...
            while (length > 0) {
                int chunk = Math.Min(Capacity - localBufferHeadOffset, length);
                destination.Write(BufferMemory, localBufferHeadOffset, chunk);
                localBufferHeadOffset = (localBufferHeadOffset + chunk == Capacity) ? 0 : localBufferHeadOffset + chunk;
                length -= chunk;
            }
            TakePublish(localBufferHeadOffset, count);
        ...
        public override async Task TakeToAsync(Stream destination, int count, CancellationToken cancellationToken)
        {
            ...
            while (length > 0) {
                int chunk = Math.Min(Capacity - localBufferHeadOffset, length);
                await destination.WriteAsync(BufferMemory, localBufferHeadOffset, chunk, cancellationToken);
                if (cancellationToken.IsCancellationRequested) {
                    return;
                }
                localBufferHeadOffset = (localBufferHeadOffset + chunk == Capacity) ? 0 : localBufferHeadOffset + chunk;
        ...
        public override void Reset()
        {
            ...
            try {
                Lock.Enter(ref lockTaken);
                // Write shared state
                Array.Clear(BufferMemory, 0, BufferMemory.Length);
                BufferHeadOffset = 0;
                BufferTailOffset = 0;
                ContentLength = 0;
            }
